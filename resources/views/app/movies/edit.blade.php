...
@section('content')
    <h1 class="text-2xl font-semibold mb-4">Editar Filme: {{ $movie->title }}</h1>

    <form method="POST" action="{{ route('movies.update', $movie) }}" class="bg-white p-6 rounded shadow">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="number" step="0.1" name="rating" id="rating" value="{{ old('rating', $movie->rating) }}"
                class="w-full border px-2 py-1 rounded">
            @error('rating')
                <p class="text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-4">
            ...
            <textarea name="review" id="review" rows="4" class="w-full border px-2 py-1 rounded">{{ old('review', $movie->review) }}</textarea>
            @error('review')
                <p class="text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            <label for="watched" class="inline-flex items-center">
                <input type="checkbox" name="watched" id="watched" value="1"
                    {{ old('watched', $movie->watched) ? 'checked' : '' }} class="mr-2">
                Assistido
            </label>
            @error('watched')
                <p class="text-red-600">{{ $message }}</p>
